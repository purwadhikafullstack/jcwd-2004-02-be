...
module.exports = {
  ...
  addToCartService: async (id, product_id, quantity) => {
    ...
    try {
      ...
      if (selectedProduct.length) {
        ...
        sql = `update cart set quantity = ? where id = ?`[result] =
          await conn.query(sql, [quantity, cart_id]);
      } else {
  ...
  getProdukTerkaitService: async (brand) => {
    ...
    try {
      ...
      left join (select name as category_name, id from category) as kategori on category_id = kategori.id
      where true and brand_name = '${brand}' and product.is_deleted = 0 group by product.id LIMIT 0,7`;
